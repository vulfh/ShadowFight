...
The feature is intentionally designed so that a future "Save named Adhoc Fight Tests" capability (analogous to today's Fight Lists) can be added with minimal rework: the `AdhocFightTest` data type defined here will be the foundation for that future feature.
...
- **Tab_Bar**: The extensible tab component added below the session controls in the Training panel. Initially contains two tabs: "Fight Lists" (default) and "Adhoc Test".
...
**User Story:** As a user, I want a tab bar below the Start/Pause/Stop buttons in the Training panel, so that I can switch between the Fight Lists workflow and the new Adhoc Test workflow.
...
2. THE Tab_Bar SHALL display a "Fight Lists" tab and an "Adhoc Test" tab at all times.
...
1. WHEN the "Fight Lists" tab is active, THE Training_Panel SHALL display the existing Fight Lists panel content inline (the list of fight lists, selection controls, and Play Mode selector).
2. THE existing Fight Lists panel in the right-hand column (col-12 col-lg-4) SHALL remain present in the DOM and continue to manage fight list data; only its display within the Training panel tab is the new integration point.
...
### Requirement 3: Adhoc Test Tab — Filter Form

**User Story:** As a user, I want to configure filter parameters in the Adhoc Test tab, so that I can define exactly which techniques are included in my session without creating a named fight list.

#### Acceptance Criteria

1. WHEN the "Adhoc Test" tab is active, THE Training_Panel SHALL display the AFT filter form with the following controls: Mode selector, Target Levels multi-select, Categories multi-select, Sides selector, and Shuffle Mode selector.
...
**User Story:** As an architect, I want a well-defined `AdhocFightTest` type that encapsulates all filter parameters, so that the system can be extended to support multiple saved Adhoc Fight Tests in the future without a type redesign.
...
4. WHERE a future multi-named-AFT feature is built, THE existing `AdhocFightTest` type and its storage format SHALL require no structural changes — only new persistence / management logic.

---

### Requirement 6: Persistence of AFT Filter Parameters

**User Story:** As a user, I want my Adhoc Test filter settings to be saved automatically, so that I do not have to re-enter them every time I open the app.
...
**User Story:** As a user, I want pressing Start while the Adhoc Test tab is active to immediately begin a session with the filtered technique set, so that I can train without manually building a fight list.

#### Acceptance Criteria

1. WHEN the "Adhoc Test" tab is Active_Tab and the user presses Start, THE Session_Controller SHALL compute the Filter_Result and call `sessionManager.startSessionWithFightList(config, fightList, playMode)` using the Filter_Result as the technique list and the configured `shuffleMode` as the play mode.
...
3. WHEN the "Adhoc Test" tab is Active_Tab and the user presses Pause or Stop, THE Session_Controller SHALL pause or stop the currently running session — identical to the existing behaviour for Fight Lists.
4. THE Start button SHALL be disabled (and an Inline_Validation_Error SHALL be shown adjacent to the Mode selector) if the user presses Start while the "Adhoc Test" tab is active and no Mode has been selected.
5. IF the Filter_Result is empty when Start is pressed on the "Adhoc Test" tab, THEN THE Notification_Service SHALL display an error notification (type `'error'`) stating that no techniques match the current filters, and the session SHALL NOT start.
...
**User Story:** As a user, I want the Adhoc Test tab to look and feel consistent with the rest of the Training panel, so that the UI is cohesive on all devices.
