...
- Voice notes are stored locally in the browser (e.g., IndexedDB) and are not synced across devices.
...
4. **Should there be a limit on the total storage size for all notes?**
   - Yes, limit to 100MB per device (per browser profile).

5. **Should the system log note-related actions (e.g., recording, deleting, playing)?**
   - Yes, for debugging and analytics.

### Session 2026-02-26
- Q: Where are voice notes stored and how are they synced across devices? → A: Stored locally per device; no sync.
